...
def branch_name(root):
    ...
    if p.returncode==1 and not p.stdout.strip() and not p.stderr.strip():
        return None
    raise RuntimeError(f'git symbolic-ref --short -q HEAD failed: {p.stderr.strip()}')

def decode_z(data):
    return [x for x in data.split(b'\0') if x]
...
def validate_policy(policy, root):
    if not isinstance(policy,dict): raise ValueError('snapshot policy must be an object')
    expected_keys={'schema_version','include_ignored','allow_sensitive_untracked'}
    if set(policy)!=expected_keys: raise ValueError('snapshot policy fields do not match schema')
    ...
    for key in ('include_ignored','allow_sensitive_untracked'):
        if not isinstance(policy[key],list): raise ValueError(f'{key} must be an array')
        seen=set()
        for value in policy.get(key,[]):
            if not isinstance(value,str): raise ValueError(f'{key} entries must be strings')
...
def index_entries(root):
    raw=git(root,'ls-files','-z','--stage')
    entries={}; submodules=[]
    for rec in decode_z(raw):
        meta,path=rec.split(b'\t',1); mode,oid,stage=meta.split(b' ',2)
        rel=rel_from_bytes(path)
...
def exact_ignored(root, policy):
    ...
    for rel in policy['include_ignored']:
        p=root/rel
        chk=subprocess.run(['git','-C',str(root),'check-ignore','-q','--',rel])
...
def actual_untracked(root):
    return [rel_from_bytes(x) for x in decode_z(git(root,'ls-files','--others','--exclude-standard','-z'))]
...
def file_record(root,rel,tracked,index_meta,archive_dir,archive_untracked):
    ...
    try: st=os.lstat(full)
        ...
        if tracked:
            return None
        raise RuntimeError(f'untracked path disappeared during snapshot: {rel}')
    ...
    if stat.S_ISLNK(st.st_mode):
        ...
        except FileNotFoundError: raise RuntimeError(f'path disappeared during snapshot: {rel}')
        record.update({'kind':'symlink','target_b64':b64(os.fsencode(target)),'content_sha256':hashlib.sha256(os.fsencode(target)).hexdigest(),'size':len(os.fsencode(target))})
    elif stat.S_ISREG(st.st_mode):
        try: digest=sha256_file(full)
        except FileNotFoundError: raise RuntimeError(f'path disappeared during snapshot: {rel}')
        record.update({'kind':'file','content_sha256':digest,'size':st.st_size})
        ...
        raise RuntimeError(f'unsupported file type: {rel}')
    return record
...
def capture_manifest(root, policy, archive_dir):
    idx=index_entries(root)
    tracked=sorted(idx.keys(),key=lambda s:os.fsencode(s))
    untracked=sorted(set(actual_untracked(root)+exact_ignored(root,policy)),key=lambda s:os.fsencode(s))
    allowed_sensitive=set(policy['allow_sensitive_untracked'])
    records=[]; absent=[]
    for rel in tracked:
    ...
    for rel in untracked:
        if is_sensitive(rel) and rel not in allowed_sensitive:
            raise RuntimeError(f'sensitive untracked path requires exact policy authorization: {rel}')
        rec=file_record(root,rel,False,None,archive_dir,True)
        records.append(rec)
    records.sort(key=lambda r:base64.b64decode(r['path_b64']))
    absent.sort(key=lambda r:base64.b64decode(r['path_b64']))
...
def ensure_safe_output(out, root, gitdir):
    ...
    raw_abs=Path(os.path.abspath(out))
    parent=out.parent.resolve()
    candidate=(parent/out.name).resolve(strict=False)
    if candidate==root or candidate==gitdir: raise RuntimeError('dangerous snapshot output path')
    # A path lexically inside the repository or its real Git management path may not escape through a symlink.
    try:
        lexical_in_repo=os.path.commonpath([str(root),str(raw_abs)])==str(root)
        lexical_in_git=os.path.commonpath([str(gitdir),str(raw_abs)])==str(gitdir)
    except ValueError:
        lexical_in_repo=lexical_in_git=False
    try:
        resolved_in_repo=os.path.commonpath([str(root),str(candidate)])==str(root)
        resolved_in_git=os.path.commonpath([str(gitdir),str(candidate)])==str(gitdir)
    except ValueError:
        resolved_in_repo=resolved_in_git=False
    ...
    # Inside repo is allowed only under the real Git management directory.
    try: in_repo=os.path.commonpath([str(root),str(candidate)])==str(root)
    except ValueError: in_repo=False
    try: in_git=os.path.commonpath([str(gitdir),str(candidate)])==str(gitdir)
    except ValueError: in_git=False
    if in_repo and not in_git: raise RuntimeError('snapshot output inside working tree is forbidden')
...
def snapshot(out, baseline_commit='', policy_path=None, repo_cwd=None):
    ...
    policy=validate_policy(load_json(policy_path) if policy_path else default_policy(),root); pd=canonical_digest(policy)
    head=git(root,'rev-parse','HEAD',text=True).strip()
    if baseline_commit:
        git(root,'rev-parse','--verify',f'{baseline_commit}^{{commit}}')
    tmp=Path(tempfile.mkdtemp(prefix=out.name+'.tmp.',dir=out.parent)); os.chmod(tmp,0o700)
    try:
        archive=tmp/'archive'
        records1,absent1,deliver1=capture_manifest(root,policy,archive)
        owner1,cached,worktree,status=status_digest(root,head,records1,absent1,pd)
        records2,absent2,deliver2=capture_manifest(root,policy,archive)
        owner2,_,_,_=status_digest(root,head,records2,absent2,pd)
        if deliver1!=deliver2 or owner1!=owner2 or records1!=records2 or absent1!=absent2:
            raise RuntimeError('repository changed during snapshot')
        branch=branch_name(root)
        manifest={'schema_version':2,'repo_root':str(root),'head':head,'branch':branch,'baseline_commit':baseline_commit or None,'policy':policy,'policy_digest':pd,'deliverable_digest':deliver1,'ownership_digest':owner1,'files':records1,'tracked_absent':absent1}
        atomic_json_write(tmp/'manifest.json',manifest)
        write_bytes(tmp/'manifest.sha256',(sha256_file(tmp/'manifest.json')+'\n').encode())
        write_bytes(tmp/'diff-cached.patch',cached); write_bytes(tmp/'diff-worktree.patch',worktree); write_bytes(tmp/'status.porcelain-v2.z',status)
        committed=b''
        if baseline_commit: committed=git(root,'diff','--binary',f'{baseline_commit}..HEAD')
        write_bytes(tmp/'diff-baseline-to-head.patch',committed)
        os.rename(tmp,out)
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('output'); ap.add_argument('baseline_commit',nargs='?',default=''); ap.add_argument('--policy')
    args=ap.parse_args()
    try:
        m=snapshot(args.output,args.baseline_commit,args.policy)
        print('SNAPSHOT_OK'); print('DELIVERABLE_DIGEST='+m['deliverable_digest']); print('OWNERSHIP_DIGEST='+m['ownership_digest']); print('POLICY_DIGEST='+m['policy_digest']); return 0
