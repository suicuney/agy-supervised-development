...
def capture(repo,out,baseline='',extra=None):
    argv=['snapshot_code_state.py',str(out)]+([baseline] if baseline else [])+(extra or []); old=os.getcwd(); os.chdir(repo)
    try:
        oldargv=sys.argv; sys.argv=argv; rc=snap.main(); sys.argv=oldargv
    finally: os.chdir(old)
    if rc: raise AssertionError(f'snapshot failed: {argv}')
    return json.loads((out/'manifest.json').read_text())
def must_fail(repo,out,extra=None):
    argv=['snapshot_code_state.py',str(out)]+(extra or []); old=os.getcwd(); os.chdir(repo)
    try:
        oldargv=sys.argv; sys.argv=argv; rc=snap.main(); sys.argv=oldargv
    finally: os.chdir(old)
    assert rc!=0
...
def main():
  with tempfile.TemporaryDirectory(prefix='agy-snapshot-test.') as td:
    ...
    (repo/'task.txt').write_text('task\n'); git(repo,'add','task.txt'); git(repo,'commit','-qm','task')
    ...
    # Root/subdir are identical; adding an unchanged untracked file to index changes ownership only.
    ...
    # Ignored acceptance input is explicit, and dangerous/unsupported states fail closed.
    ...
    (repo/'.env').write_text('SECRET=x'); must_fail(repo,gitdir/'sensitive'); allowed=capture(repo,gitdir/'sensitive-ok',extra=['--allow-sensitive-untracked','.env']); assert allowed
    ...
    # Concurrency detection: second observed manifest differs -> no successful snapshot.
    original=snap.capture_manifest; calls={'n':0}
    def unstable(*args,**kwargs):
        records,digest=original(*args,**kwargs); calls['n']+=1
        return (records, digest if calls['n']==1 else '0'*64)
    snap.capture_manifest=unstable; must_fail(repo,gitdir/'unstable'); snap.capture_manifest=original
